...
class VoiceShoppingAssistant {
    ...
    setupEventListeners() {
        // Microphone button
        const micBtn = document.getElementById('voice-activation-btn');
        ...
        // Debug controls
        const testMicBtn = document.getElementById('test-mic-access');
        if (testMicBtn) {
            testMicBtn.addEventListener('click', () => this.testMicrophone());
        }

        const debugInfoBtn = document.getElementById('show-debug-info');
        if (debugInfoBtn) {
            debugInfoBtn.addEventListener('click', () => this.showDebugInfo());
        }

        const retryVoiceBtn = document.getElementById('retry-voice-service');
        if (retryVoiceBtn) {
            retryVoiceBtn.addEventListener('click', () => this.retryVoiceRecognition());
        }

        const testCommandBtn = document.getElementById('test-processing-btn');
        if (testCommandBtn) {
    ...
    showSuccessFeedback() {
        ...
        const shoppingSection = document.querySelector('.list-display-panel');
        if (shoppingSection) {
    ...
    displaySearchResults(results, query) {
        ...
        if (results.length === 0) {
            resultsContainer.innerHTML = `<div class="list-empty-state">No products found for "${query}"</div>`;
            return;
        }
    ...
    generateSuggestions() {
        ...
        const suggestionsHTML = suggestions.map(suggestion => `
            <div class="suggestion-card">
                <span class="suggestion-emblem">${suggestion.icon}</span>
    ...
    updateUI() {
        ...
        if (this.shoppingList.length === 0) {
            listContainer.innerHTML = '<div class="list-empty-state">Your list is empty. Try saying "add milk" or "add 2 apples"</div>';
            return;
        }
    ...
    updateMicButton(listening, state = 'listening') {
        const micBtn = document.getElementById('voice-activation-btn');
        const micText = micBtn.querySelector('.btn-text');
        ...
        if (listening) {
            ...
            micText.textContent = 'Press to Speak';
        }
    }
    ...
    handleNetworkError() {
        ...
        // Show retry button
        const retryBtn = document.getElementById('retry-voice-service');
        if (retryBtn) {
